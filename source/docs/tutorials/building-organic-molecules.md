...
The [Template Tool](tools-template-tool) provides shortcuts for building alkyl chains without drawing each carbon. Let's build a simple fatty acid: decanoic acid.

**Start with a carboxylic acid**

Draw a single carbon with the [Draw Tool](tools-draw-tool), or start with methane from **Build → Insert → Molecule...**

**Add the carboxylic acid**

In the [Template Tool](tools-template-tool) **{ref}`Groups tab<template-tool-groups-tab>`, type `C` (uppercase) or `co2` to select carboxylate. Click on a hydrogen to add the -COOH group. We now have acetic acid.
...
Build isobutylbenzene (a simple branched aromatic):
...
3. Add nitro group: Type `N` and click on an adjacent hydrogen (position 3, ortho to COOH)
4. Add methoxy group: Type `om` and click on the para hydrogen (position 4, opposite COOH)
...
- [Insert Fragments](building-molecules-insert-fragments) for inserting complete molecular fragments
